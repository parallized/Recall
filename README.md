...
> [!WARNING]
> **免责声明 (Disclaimer) - 快速迭代中**
> 本项目当前正处于高度活跃的 **Alpha 快速迭代期**。底层的拓扑存储引擎、SQLite Schema 架构、以及 MCP 大模型代理与网关接口随时可能发生无法向后兼容的 Breaking Changes。
> 在未全面克隆与备份您的本地知识数据库之前，请务必不要将此阶段的代码分支用于任何严肃的生产环境。
