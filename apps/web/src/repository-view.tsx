...
const UnreadBadge = ({ count }: { count: number }) => {
  ...
  return (
    <span className="inline-flex min-w-4 items-center justify-center rounded-sm bg-black px-1 py-0.5 text-[8px] font-black text-white tabular-nums">
      {count}
    </span>
  );
};
    ...
    <div className="space-y-[1px]">
      ...
      >
        {isSelected && (
           <motion.div 
             ...
             className="absolute left-0 top-1/2 -translate-y-1/2 w-[2px] h-4 bg-black z-10 rounded-full" 
           />
        )}
        ...
        <button
          ...
          className={cn(
            ...
            hasChildren && "text-black/10 hover:text-black/60"
          )}
        >
          <ChevronRight 
            className={cn(
      ...
      <AnimatePresence initial={false}>
        {hasChildren && isExpanded && (
          ...
          >
            <div className="absolute left-0 top-0 bottom-2 w-px bg-black/[0.03]" />
...
const RepositoryTruthCard = ({
  ...
  <motion.button
    ...
    onClick={onSelect}
    className="w-full px-16 py-4 text-left transition-colors border-b border-black/[0.03] relative group hover:bg-black/[0.01]"
  >
    <div className="flex items-center gap-4">
      <div className="flex-1 min-w-0 flex items-center gap-3">
        {truth.isUnread && (
          <div className="w-1.5 h-1.5 rounded-full bg-black shrink-0" title="未读" />
        )}
        <div className={cn(
    ...
    {isOpen && (
      <>
        <motion.div
          ...
          exit={{ opacity: 0 }}
          onClick={onClose}
          className="fixed inset-0 bg-white/60 backdrop-blur-md z-[60] cursor-zoom-out"
        />
        ...
        <motion.div
          ...
          className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[95%] max-w-3xl max-h-[90vh] z-[70] isolate"
        >
          <div className="bg-white border border-black/[0.05] shadow-[0_40px_100px_rgba(0,0,0,0.1)] rounded-none flex flex-col h-full transform-gpu relative">
             ...
             <div className="flex-1 overflow-y-auto custom-scrollbar px-16 pb-20 pt-4">
  ...
  if (!truth) {
    return (
      <div className="h-full flex flex-col items-center justify-center space-y-10 animate-in fade-in duration-1000">
        <div className="w-24 h-24 bg-black/[0.02] flex items-center justify-center border border-black/[0.03]">
    ...
    <div className="h-full animate-in fade-in duration-800 space-y-14">
      <header className="space-y-6">
         <div className="flex items-center gap-4 text-ink/30 text-[9px] font-black tracking-[0.3em] uppercase">
            <span className="text-black">{questionTypeLabel[truth.questionType ?? "open_ended"]}</span>
      ...
      <div className="space-y-14">
                ...
                {truth.options.map((option, index) => {
                  ...
                  return (
                    <div key={`${truth.id}-${option}`} className={cn(
                      ...
                      isAnswer ? "bg-black text-white border-black" : "bg-white border-black/[0.06] text-ink/60"
                    )}>
        ...
        {truth.explanation && (
          <section className="space-y-6 bg-black/[0.015] p-8 border border-black/[0.03]">
             <h4 className="text-[10px] font-black uppercase tracking-[0.5em] text-ink/20">延伸洞察</h4>
        ...
        <section className="space-y-6 py-8 border-t border-black/[0.05]">
           <div className="flex items-center justify-between gap-4">
              ...
              <a href={truth.sourceUrl} target="_blank" rel="noreferrer" className="flex items-center gap-3 text-[11px] font-black uppercase tracking-[0.3em] text-ink/60 hover:text-black transition-colors">
                 <span>信源链接</span>
                 <ArrowUpRight className="w-4 h-4" />
              </a>
           </div>

           <div className="flex items-center justify-between gap-4 border-t border-black/[0.05] pt-6">
              <div className="text-[11px] font-black tracking-[0.2em] text-ink/30 uppercase">
              ...
              <div className="flex items-center gap-8">
                 ...
                 <button
                   ...
                   disabled={destroying || markingRead}
                   className="text-[12px] font-black tracking-widest uppercase text-ink/60 hover:text-black"
                 >
...
export const RepositoryView = ({
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="space-y-6 text-center">
          <div className="mx-auto flex h-20 w-20 items-center justify-center bg-black/[0.02] border border-black/[0.05]">
  ...
  if (error) {
    return (
      <div className="max-w-xl mx-auto border border-black/[0.05] bg-white p-16 text-center space-y-8">
        ...
        <button
          ...
          className="inline-flex items-center gap-4 border border-black px-10 py-3 text-[11px] font-black uppercase tracking-widest transition-all hover:bg-black hover:text-white"
        >
  ...
  if (!snapshot || snapshot.summary.truthCount === 0) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center bg-white">
        <div className="max-w-md border border-black/[0.05] bg-white p-20 text-center space-y-10 group">
          <div className="mx-auto w-24 h-24 bg-black/[0.01] border border-black/[0.03] flex items-center justify-center group-hover:scale-110 transition-transform duration-700">
  ...
  return (
    <div className="h-screen bg-white overflow-hidden flex flex-col relative animate-in fade-in duration-1000">
      <div className="flex-1 flex overflow-hidden">
        <aside className="w-[320px] bg-white border-r border-black/[0.05] flex flex-col shrink-0 overflow-hidden relative">
          <header className="px-10 pt-10 pb-6 relative">
             ...
             <p className="text-ink/20 text-[10px] mt-2 font-black leading-relaxed tracking-[0.3em] uppercase">知识仓库 • REPOSITORY</p>
          </header>

          <div className="flex-1 flex flex-col min-h-0 px-4">
             <div className="px-6 pb-4 flex items-center justify-between border-b border-black/[0.03] mb-4">
                <h3 className="text-[10px] font-black text-ink/10 uppercase tracking-[0.4em]">索引节点</h3>
                <div className="text-[10px] font-black text-ink/10 tabular-nums">{snapshot.taxonomy.length}</div>
             </div>
             ...
             <div className="flex-1 overflow-y-auto custom-scrollbar px-2 pb-10">
                <div className="space-y-1">
                  <button
                    ...
                    className={cn(
                      "w-full text-left px-5 py-3 transition-all relative group mb-2",
                      selectedTagId === null ? "bg-black/[0.03]" : "hover:bg-black/[0.015]"
                    )}
                  >
                    {selectedTagId === null && (
                      <motion.div 
                        layoutId="sidebar-active-pill"
                        className="absolute left-0 top-1/2 -translate-y-1/2 w-[2px] h-5 bg-black z-10" 
                      />
                    )}
                    <div className="flex items-center justify-between gap-3">
                       <div className="flex min-w-0 items-center gap-3 pl-8">
                         <span className={cn("text-[12px] font-bold tracking-widest uppercase", selectedTagId === null ? "text-ink" : "text-ink/30")}>全量索引</span>
                         <UnreadBadge count={snapshot.summary.unreadCount} />
                       </div>
                       <span className="text-[8px] font-black opacity-10 group-hover:opacity-30 transition-opacity tabular-nums">{snapshot.summary.truthCount}</span>
                    </div>
                  </button>
                  <div className="h-px bg-black/[0.03] mx-5 my-4" />
        ...
        <div className="flex-1 flex flex-col h-screen max-h-screen border-r border-black/[0.05] bg-white relative">
          <header className="px-16 pt-10 pb-6">
             <div className="flex items-start justify-between gap-10">
                ...
                <div className="flex items-center gap-6 pt-2 shrink-0">
                   <button
                     ...
                     disabled={refreshing}
                     className="p-3 border border-black/[0.08] text-ink/30 hover:text-black transition-all hover:bg-black/[0.02]"
                   >
                   ...
                   <button
                     ...
                     className="px-8 py-3 bg-black text-white text-[11px] font-black uppercase tracking-[0.3em] transition-all hover:translate-y-[-2px] hover:shadow-[0_10px_30px_rgba(0,0,0,0.15)] disabled:opacity-20"
                   >
                     扩充题库
                   </button>
                </div>
             ...
             <div className="relative mt-6 group">
                <Search className="pointer-events-none absolute left-0 top-1/2 h-4 w-4 -translate-y-1/2 text-ink/10 transition-colors group-focus-within:text-ink/60" />
                <input
                  value={searchQuery}
                  onChange={(event) => setSearchQuery(event.target.value)}
                  placeholder="检索知识引擎中的事实细则..."
                  className="w-full h-12 bg-transparent border-b border-black/[0.05] py-4 pl-8 pr-4 text-[13px] text-ink outline-none transition-all placeholder:text-ink/10 focus:border-black tracking-widest uppercase font-bold"
                />
             </div>

             {selectedTag === null && (
               <div className="mt-4 text-[9px] font-black uppercase tracking-[0.3em] text-ink/15">请先在左侧选择分类节点以发起扩充协议。</div>
             )}
             {expandError && <div className="mt-4 text-[10px] font-bold text-ember uppercase">{expandError}</div>}
             {!expandError && visibleUnreadCount > 0 && (
               <div className="mt-4 text-[10px] font-black uppercase tracking-[0.4em] text-ink/20">待处理条目 / {visibleUnreadCount}</div>
             )}
          </header>
